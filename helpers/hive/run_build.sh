#!/bin/bash
# This script compiles Hive code using pre-built Docker image (Maven project)
set -e # Exit on error

echo "--- Building Hive for commit ${COMMIT_SHA:0:7} ---"
...
docker volume create maven-cache-hive 2>/dev/null || true
...
if docker run --rm \
    --dns=8.8.8.8 \
    -v "${PROJECT_DIR}:/repo" \
    -v "maven-cache-hive:/root/.m2" \
    -w /repo \
    ${BUILDER_IMAGE_TAG} \
    bash -c "rm -rf /root/.m2/repository/org/apache/hive && ${BUILD_COMMAND}"; then
